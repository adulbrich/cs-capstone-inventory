...
### Route Structure
All routes follow SvelteKit's file-based routing:
- `+page.svelte`: UI component
- `+page.server.ts`: Server-side load + form actions
- `+layout.server.ts` / `+layout.svelte`: Shared layouts with nav

### Form Actions Pattern
Heavy use of **progressive enhancement** via form actions (not API routes). Actions return `{ success: boolean, message?: string }` or use `fail()`:
...
export const actions = {
  actionName: async ({ request, locals: { supabase } }) => {
    ...
    return { success: true }
    // OR
    return fail(500, { success: false, message: "Error description" })
  }
} satisfies Actions
```
#### Svelte 5 Runes
Use modern Svelte 5 syntax:
- `let { data, children } = $props()` for component props
- `let { session, supabase } = $derived(data)` for reactive derivations
- `{@render children()}` for slot content
- `$page.url.pathname` for routing state (from `$app/state`)
